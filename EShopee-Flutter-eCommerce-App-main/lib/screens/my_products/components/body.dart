...
import 'package:e_commerce_app_flutter/components/product_detail_card.dart';
        ...
        if (direction == DismissDirection.startToEnd) {
          ...
          if (confirmation) {
            ...
            try {
              ...
              final result = UserDatabaseHelper().removeAllCartProductFromCart(product.id);
              productInfoDeleted = await showDialog(
                context: context,
                builder: (context) {
                  return AsyncProgressDialog(
                    result,
                    message: Text("Deleting Product carts"),
                  );
                },
              );

              final deleteProductFuture =
                  ProductDatabaseHelper().deleteUserProduct2(product.id);
              productInfoDeleted = await showDialog(
                context: context,
                builder: (context) {
                  return AsyncProgressDialog(
