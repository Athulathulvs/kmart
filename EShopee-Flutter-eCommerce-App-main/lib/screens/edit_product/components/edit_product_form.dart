...
class _EditProductFormState extends State<EditProductForm> {
  ...
  final TextEditingController quantityUnitFieldController = TextEditingController();
  ...
  void dispose() {
    ...
    quantityUnitFieldController.dispose();
  ...
  void initState() {
    ...
    if (widget.product == null) {
      ...
      productDetails.initialSelectedImages = widget.product.images
          ...
          .toList();
      productDetails.initialSelectedCertificate = widget.product.certificate
          .map((e) => CustomImage(imgType: ImageType.network, path: e))
          .toList();
  ...
  Widget build(BuildContext context) {
    final column = Column(
      children: [
        ...
        buildUploadCertificateImagesTile(context),
        SizedBox(height: getProportionateScreenHeight(20)),
    ...
    if (newProduct == false) {
      ...
      quantityUnitFieldController.text = product.quantityUnit;
  ...
  bool validateBasicDetailsForm() {
    if (_basicDetailsFormKey.currentState.validate()) {
      ...
      product.quantityUnit = quantityUnitFieldController.text;
      product.quantity = double.parse(quantityFieldController.text);
      return true;
    }
    return false;
  }
      ...
      child: Consumer<ProductDetails>(
        builder: (context, productDetails, child) {
          return DropdownButton<ProductType>(
            value: ProductType.Cardamon,
            items: ProductType.values
                .map(
                  (e) => DropdownMenuItem<ProductType>(
                    value: e,
                    child: Text(
                        ...
                        child: productDetails.selectedImages[index].imgType ==
                            ...
                            ? Image.memory(
                                File(productDetails.selectedImages[index].path)
                                    .readAsBytesSync())
                            : productDetails.selectedImages[index].path != null ? Image.network(
                                productDetails.selectedImages[index].path) :  Text("Image  path null"),
                      ),
                    ),
                  ),
                ),
  ...
  Widget buildUploadCertificateImagesTile(BuildContext context) {
    return ExpansionTile(
      title: Text(
        "Upload Certificate",
        style: Theme.of(context).textTheme.headline6,
        ...
        Consumer<ProductDetails>(
          builder: (context, productDetails, child) {
            return Row(
              ...
              children: [
                ...List.generate(
                  productDetails.selectedCertificate.length,
                      (index) => SizedBox(
                    width: 80,
                    height: 80,
                    child: Padding(
                      ...
                      child: GestureDetector(
                        onTap: () {
                          addCertificateButtonCallback(index: index);
                        },
                        child: productDetails.selectedCertificate[index].imgType ==
                            ImageType.local
                            ? Image.memory(
                            File(productDetails.selectedCertificate[index].path)
                                .readAsBytesSync())
                            : productDetails.selectedCertificate[index].path != null ? Image.network(
                            productDetails.selectedCertificate[index].path) :  Text("Image  path null"),
                      ),
                    ),
                  ),
                ),
  ...
  Widget buildQuantityUnitField() {
    return TextFormField(
      controller: quantityUnitFieldController,
      decoration: InputDecoration(
        hintText: "KG/Ton/Gram",
        labelText: "Quantity Unit",
        floatingLabelBehavior: FloatingLabelBehavior.always,
      ),
      validator: (_) {
        if (quantityUnitFieldController.text.isEmpty) {
          return FIELD_REQUIRED_MSG;
        }
        return null;
      },
  ...
  Widget buildDescriptionField() {
    return TextFormField(
      ...
      decoration: InputDecoration(
        hintText:
            "e.g., cardamom, also spelled cardamon, spice consisting of whole or ground dried fruits, or seeds, of Elettaria cardamomum, a herbaceous perennial plant of the ginger family (Zingiberaceae).",
  ...
  Future<void> saveProductButtonCallback(BuildContext context) async {
    ...
    if (productDetails.selectedCertificate.length < 1) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text("Upload Certificate Image"),
        ),
      );
      return;
    }
    ...
    try {
      ...
      certificateUploaded = await uploadProductCertificateImages(productId);
      if (allImagesUploaded == true && certificateUploaded == true) {
    ...
    List<String> downloadCerUrls = productDetails.selectedCertificate
        .map((e) => e.imgType == ImageType.network ? e.path : null)
        .toList();
    bool productFinalizeUpdate = false;
    bool productCerFinalizeUpdate = false;
    try {
      ...
      productFinalizeUpdate = await showDialog(
        ...
        builder: (context) {
          return AsyncProgressDialog(
            updateProductFuture,
            message: Text("Saving Product Image"),
          );
        },
      );

      final updateCerProductFuture =
      ProductDatabaseHelper().updateProductsCertificateImages(productId, downloadCerUrls);
      productCerFinalizeUpdate = await showDialog(
        context: context,
        builder: (context) {
          return AsyncProgressDialog(
            updateCerProductFuture,
            message: Text("Saving Product Certificate"),
          );
        },
      );

      if (productFinalizeUpdate == true && productCerFinalizeUpdate == true) {
  ...
  Future<bool> uploadProductCertificateImages(String productId) async {
    bool allImagesUpdated = true;
    final productDetails = Provider.of<ProductDetails>(context, listen: false);
    for (int i = 0; i < productDetails.selectedCertificate.length; i++) {
      if (productDetails.selectedCertificate[i].imgType == ImageType.local) {
        print("Image being uploaded: " + productDetails.selectedCertificate[i].path);
        String downloadUrl;
        try {
          final imgUploadFuture = FirestoreFilesAccess().uploadFileToPath(
              File(productDetails.selectedCertificate[i].path),
              ProductDatabaseHelper().getPathForProductCertificateImage(productId, i));
          downloadUrl = await showDialog(
            context: context,
            builder: (context) {
              return AsyncProgressDialog(
                imgUploadFuture,
                message: Text(
                    "Uploading Images ${i + 1}/${productDetails.selectedCertificate.length}"),
              );
            },
          );
        } on FirebaseException catch (e) {
          ...
          if (downloadUrl != null) {
            productDetails.selectedCertificate[i] =
                CustomImage(imgType: ImageType.network, path: downloadUrl);
          } else {
            allImagesUpdated = false;
            ScaffoldMessenger.of(context).showSnackBar(
  ...
  Future<void> addCertificateButtonCallback({int index}) async {
    final productDetails = Provider.of<ProductDetails>(context, listen: false);
    if (index == null && productDetails.selectedCertificate.length >= 1) {
      ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text("Max 1 images can be uploaded")));
      return;
    }
    String path;
    String snackbarMessage;
    ...
    if (index == null) {
      productDetails.addNewSelectedCertificate(
          CustomImage(imgType: ImageType.local, path: path));
    } else {
      productDetails.setSelectedCertificateAtIndex(
          CustomImage(imgType: ImageType.local, path: path), index);
    }
  }
}
