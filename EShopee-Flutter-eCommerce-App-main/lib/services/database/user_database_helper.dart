...
class UserDatabaseHelper {
  ...
  static const String SMART_FARM_CONFIG = "smart_farm_config";
  ...
  Future<void> createNewUser(String uid,String email) async {
    await firestore.collection(USERS_COLLECTION_NAME).doc(uid).set({
      ...
      SMART_FARM_CONFIG: null,
      SMART_FARM_CLIENT_ID: null,
      EMAIL: email,
      NAME: email,
    });
  }
  ...
  Future<List<OrderproductList>> emptyCart() async {
    String uid = AuthentificationService().currentUser.uid;
    ...
    List<OrderproductList> orderedProductsUid = List<OrderproductList>();
    for (final doc in cartItems.docs) {
      final userDocData = doc.data();
      final quantity = userDocData[CartItem.ITEM_COUNT_KEY];

      final docRef = productsCollectionReference.doc(doc.id);
      await docRef.update({
        Product.QUANTITY: FieldValue.increment(-quantity),
      });
      final docSnapshot = await docRef.get();
      final price = docSnapshot.data()[Product.DISCOUNT_PRICE_KEY];
      orderedProductsUid.add(new OrderproductList(
          productUid: doc.id.toString(),
  ...
  Future<bool> removeAllCartProductFromCart(String productId) async {
    //String uid = AuthentificationService().currentUser.uid;
    final cartProductCollectionReference = firestore
        .collection(USERS_COLLECTION_NAME);
    final docSnapshot = await cartProductCollectionReference.get();

    for (final item in docSnapshot.docs) {
      final userDocSnapshot = firestore.collection(USERS_COLLECTION_NAME).doc(item.id);
      await userDocSnapshot.collection(CART_COLLECTION_NAME).doc(productId).delete();
  ...
  Future<bool> updateQuantityCartItemCount(String cartItemID,int qnty) async {
    String uid = AuthentificationService().currentUser.uid;
    final cartCollectionRef = firestore
        .collection(USERS_COLLECTION_NAME)
        .doc(uid)
        .collection(CART_COLLECTION_NAME);
    final docRef = cartCollectionRef.doc(cartItemID);
    docRef.update({CartItem.ITEM_COUNT_KEY: qnty});
    return true;
  }
...
/*  Future<OrderedProduct> getOrderedProductFromId(String id) async {
  ...
  Future<bool> updateCurrentUserDetails(String name) async {
    String uid = AuthentificationService().currentUser.uid;
    final userDocSnapshot =
    firestore.collection(USERS_COLLECTION_NAME).doc(uid);
    await userDocSnapshot.update({NAME: name});
    return true;
  }
